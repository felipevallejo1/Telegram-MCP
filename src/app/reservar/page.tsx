...
export default async function Reserve({ searchParams }: { searchParams: Promise<{ specialistId?: string; slotId?: string; error?: string }> }) {
  ...
  if (!patient) return <><SiteHeader /><main className="narrow"><h1>Sesión requerida</h1><p>Ingresá antes de reservar.</p><Link className="button" href="/login">Ingresar</Link></main></>;
  ...
  return <><SiteHeader /><main className="narrow"><h1>Confirmar turno</h1>{p.error && <p role="alert" className="error">{p.error}</p>}{!slot ? <><p className="empty">El horario ya no está disponible.</p><Link href="/">Buscar otro horario</Link></> : <form action={reserveDemoAction} className="stack"><p><strong>{slot.specialist.firstName} {slot.specialist.lastName}</strong><br />{slot.specialist.specialty.name} · {modalityLabel(slot.specialist.modality)}<br />{formatBuenosAires(slot.startsAt)}</p><input type="hidden" name="specialistId" value={slot.specialistId} /><input type="hidden" name="slotId" value={slot.id} /><label className="check"><input type="checkbox" name="confirmed" required />Confirmo reservar este turno para {patient.firstName}.</label><button className="button">Confirmar reserva</button></form>}</main></>;
}
